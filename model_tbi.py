...
class natorderNet(nn.Module):
    ...
    def forward(self, imgfeature, captions, lengths, orders, tgt_len, doc_num):
        document_matrix, paramemory, key, sen_mask, pos_emb, im_mean, sen_mean = self.encode(imgfeature, captions, lengths, doc_num)

        order = torch.tensor(orders).unsqueeze(0).cuda()
        batch, num = order.size()

        dec_outputs = self.decoder(pos_emb, paramemory, sen_mask, sen_mask)
    ...
    def encode(self, imgfeature, captions, lengths, doc_num):
        ...
        pos_imags = positional_encodings_like(imags, t=None)
        pos_imag = self.linears[3](pos_imags)
        ...
        paraim = self.encoder2(imags + pos_imag, ima_mask)

        biattention = self.biatten.forward_all(parasen, paraim)
        biatte = torch.mean(biattention.squeeze(0), dim=0, keepdim=True).squeeze(0)
        biatten = F.softmax(biatte, dim=-1)
        posi_attention = torch.mm(biatten, self.linears[2](pos_imags).squeeze(0))

        pos_emb = positional_encodings_like(sentences, t=None)

        para_memory = parasen + posi_attention.unsqueeze(0)
        ...
        return sentences, para_memory, key, sen_mask, pos_emb, im_mean, sen_mean
...
def nat_pointer(args, model, imgfeature, captions, lengths, doc_num):
    sentences, paramemory, keys, sen_mask, pos_emb, im_mean, sen_mean = model.encode(imgfeature, captions, lengths, doc_num)

    dec_output = model.decoder(pos_emb, paramemory, sen_mask, sen_mask)
...
def train(args, train_iter, dev, test_real, doc, checkpoint):
    ...
    for epc in range(args.maximum_steps):
        ...
        for iters, (image_stories, captions_set, lengths_set, order_set, tgt_len_set, doc_num_set) in enumerate(train_iter):
            ...
            img_feature = model.img_enc(images) #b*
            ...
            for si, data in enumerate(zip(img_feature, captions_set, lengths_set, order_set, tgt_len_set, doc_num_set)):
                ...
                shuffle(randid)
                caption = torch.stack([captions[ri] for ri in randid])
                length = torch.tensor([lengths[ri] for ri in randid])
                order = [orders[ri] for ri in randid]
                order = list(np.argsort(order))

                loss1, im_mean, sen_mean = model(imgfeature, caption, length, order, tgt_len, doc_num)
